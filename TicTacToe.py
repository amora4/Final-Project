...
class Player:
    ...
    def setLetter(self, letterSelection):
...
class Game:
    ...
    def inputPlayerLetter(self):
        ...
        if letter == 'O':
            ...
            print('')
            # return ['O', 'X']
            self.player1.setLetter('O')
            self.player2.setLetter('X')
        elif letter == 'X':
            print('player1 is X')
            print('player2 will be O')
            print('')
            # return ['X', 'O']
            self.player1.setLetter('X')
            self.player2.setLetter('O')
        ...
        while True:
           ...
           while gameIsPlaying:
               ...
               if turn == self.player1:
                   # Player1s turn.
                   self.showBoard()
                   move = self.player1.playerMove(self.board)
                   #if move position is free then make move on board else move again
                   while True:
                       if self.board.isSpaceFree(move):
                           break
                       else:
                           #move again
                           print('Position ' + str(move) + ' is already taken, move again')
                           move = self.player1.playerMove(self.board)
                   ...
                   # Player2s turn.
                   self.showBoard()
                   move = self.player2.playerMove(self.board)
                   #if move position is free then make move on board else move again
                   while True:
                       if self.board.isSpaceFree(move):
                           break
